...
def make_browse_request(
    settings, search_criteria, game_key, challenge_key, challenge_data
):
...
def verify_challenge_reply(stream, game_key, challenge_key, challenge_data):
    ...
    if version != expected:
        logger.warning(
            "Unexpected version in challenge reply header (%i != %i)", version, expected
        )
        return False
    ...
    if enabled and game_key is None:
        logger.warning(
            "Received encrypted challenge reply but no game key was provided"
        )
        return False
...
class LanSessionSearchCriteria:
    ...
    def calc_flags(self):
        ...
        if self.max_participants is not None:
            flags |= 2
        if self.opened_only is not None:
            flags |= 4
        if self.vacant_only is not None:
            flags |= 8
    ...
    def decode(self, stream):
        ...
        attrib_values = stream.repeat(lambda: stream.repeat(stream.u32, 20), 6)
        ...
        for i in range(6):
            if flags & (64 << i):
                if attrib_mode[i]:
                    self.attributes[i] = types.Range(
                        attrib_range_min[i], attrib_range_max[i]
                    )
                else:
...
class LanSessionInfo:
    ...
    def decode(self, stream):
        ...
        self.players = stream.repeat(lambda: stream.extract(LanStationInfo), 16)
        if stream.settings["pia.lan_version"] >= 1:
...
class LanServer:
    ...
    def process_challenge(self, stream):
        ...
        if crypto:
            ...
            response += generate_challenge_reply(
                self.game_key, new_key + key, challenge, nonce
            )
        else:
...
async def browse(settings, search_criteria, key=None, timeout=1, max=0):
    ...
    async with udp.bind() as sock:
        request = make_browse_request(
            settings, search_criteria, key, challenge_key, challenge_data
        )
        ...
        async with anyio.move_on_after(timeout):
            while max == 0 or len(sessions) < max:
                ...
                session = parse_browse_reply(
                    settings, data, key, challenge_key, challenge_data
                )
