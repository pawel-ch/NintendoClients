...
class MatchmakeSessionSearchCriteria(common.Structure):
    ...
    def check_required(self, settings):
        ...
        if settings["nex.version"] >= 40000:
            for field in [
                "exclude_user_password",
                "exclude_system_password",
                "refer_gid",
                "codeword",
            ]:
                if getattr(self, field) is None:
...
class MatchMakingClient(MatchMakingProtocol):
    ...
    async def add_partitipants(self, gid, pids, message):
        ...
        stream.string(message)
        data = await self.client.request(
            self.PROTOCOL_ID, self.METHOD_ADD_PARTITIPANTS, stream.get()
        )
...
class MatchmakeExtensionClient(MatchmakeExtensionProtocol):
    ...
    async def create_matchmake_session(
        self, gathering, description, participation_count
    ):
    ...
    async def get_my_black_list(self):
        ...
        stream = streams.StreamOut(self.settings)
        data = await self.client.request(
            self.PROTOCOL_ID, self.METHOD_GET_MY_BLACK_LIST, stream.get()
        )
    ...
    async def add_to_black_list(self, pids):
        ...
        stream.list(pids, stream.pid)
        data = await self.client.request(
            self.PROTOCOL_ID, self.METHOD_ADD_TO_BLACK_LIST, stream.get()
        )
    ...
    async def remove_from_black_list(self, pids):
        ...
        stream.list(pids, stream.pid)
        data = await self.client.request(
            self.PROTOCOL_ID, self.METHOD_REMOVE_FROM_BLACK_LIST, stream.get()
        )
    ...
    async def clear_my_black_list(self):
        ...
        stream = streams.StreamOut(self.settings)
        data = await self.client.request(
            self.PROTOCOL_ID, self.METHOD_CLEAR_MY_BLACK_LIST, stream.get()
        )
    ...
    async def join_matchmake_session_ex(
        self, gid, gmessage, ignore_block_list, participation_count
    ):
...
class MatchmakeExtensionServer(MatchmakeExtensionProtocol):
    ...
    async def handle_create_matchmake_session(self, client, input, output):
        ...
        response = await self.create_matchmake_session(
            client, gathering, description, participation_count
        )
    ...
    async def handle_join_matchmake_session_ex(self, client, input, output):
        ...
        response = await self.join_matchmake_session_ex(
            client, gid, gmessage, ignore_block_list, participation_count
        )
    ...
    async def remove_from_black_list(self, *args):
        logger.warning(
            "MatchmakeExtensionServer.remove_from_black_list not implemented"
        )
        raise common.RMCError("Core::NotImplemented")
