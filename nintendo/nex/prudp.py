...
from anynet import udp, tls, websocket, util, scheduler, crypto, streams, queue
import contextlib
import hashlib
import struct
import random
...
class RC4Encryption:
    ...
    def encrypt(self, data):
        return self.rc4enc.crypt(data)

    def decrypt(self, data):
        return self.rc4dec.crypt(data)
...
class PRUDPClient:
    ...
    async def resend_packet(self, packet, counter):
        ...
        if counter < self.resend_limit:
            ...
            handle = await self.scheduler.schedule(
...
@contextlib.asynccontextmanager
async def connect(
    settings, host, port, vport=1, type=10, context=None, credentials=None
):
...
@contextlib.asynccontextmanager
async def serve(
    handler, settings, host="", port=0, vport=1, type=10, context=None, key=None
):
