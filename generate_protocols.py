...
class Parser:
    ...
    def parse_constant(self, stream, type):
        if type.name in NUMERIC_TYPES + ["datetime"]:
    ...
    def parse_struct_item(self, stream):
        ...
        if token.type == TYPE_SYMBOL:
            if token.value == "[":
                return self.parse_conditional(stream)
            elif token.value == "#":
                return self.parse_revision(stream)
            else:
                stream.error(token)
        else:
...
class CodeGenerator:
    ...
    def generate_server_method(self, stream, proto, method):
        ...
        if not method.supported:
            stream.write_line(
                '\tlogger.warning("%s.%s is unsupported")' % (class_name, method.name)
            )
            stream.write_line('\traise common.RMCError("Core::NotImplemented")')
            return
...
class DocsGenerator:
    ...
    def format_type(self, type):
        ...
        if type.name == "variant":
            return "object"
        if type.name == "datetime":
            return "[DateTime](../common#datetime)"
        if type.name == "stationurl":
            return "[StationURL](../common#stationurl)"
        if type.name == "result":
            return "[Result](../common#result)"
        if type.name == "anydata":
            return "[Data](../common)"
        if type.name == "ResultRange":
            return "[ResultRange](../common#resultrange)"
        if type.name == "NotificationEvent":
            return "[NotificationEvent](../notification#notificationevent)"
        if type.name in self.file.structs:
